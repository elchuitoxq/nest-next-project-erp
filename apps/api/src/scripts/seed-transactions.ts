...
async function seedTransactions() {
  ...
  try {
    ...
    const adminUser = await db.query.users.findFirst({ where: eq(users.email, 'admin@erp.com') });
    if (!adminUser) throw new Error('Admin user not found. Run basic seed first.');

    const allBranches = await db.query.branches.findMany();
    const allPartners = await db.query.partners.findMany();
    const allProducts = await db.query.products.findMany({ where: eq(products.type, 'PHYSICAL') });
    ...
    const getRateForDate = (date: Date) => {
        ...
        const rate = ratesHistory.find(r => r.date && r.date.toISOString().slice(0, 10) === dateStr);
    ...
    for (let i = 0; i < 20; i++) {
        ...
        try {
            ...
            // Backdate Order
            await db.update(orders).set({ date: date }).where(eq(orders.id, order.id));

            // Confirm (Moves Stock)
            const confirmedOrder = await ordersService.confirm(order.id, adminUser.id);
            ...
            const { invoice } = await ordersService.generateInvoice(confirmedOrder.id, adminUser.id);

            // Backdate Invoice
            await db.update(invoices).set({ date: date, exchangeRate: dailyRate }).where(eq(invoices.id, invoice.id));
            ...
            if (Math.random() > 0.3) { // 70% Paid
                const amount = Number(postedInvoice.total); // Total in VES
                const branchMethods = allMethods.filter(m => m.branchId === branch.id);
                const method = faker.helpers.arrayElement(branchMethods);
                const account = allAccounts.find(a => a.currencyId === method.currencyId && a.branchId === branch.id);

                if (account) {
                    const payment: any = await db.insert(payments).values({
                    ...
                    await db.update(invoices).set({ status: 'PAID' }).where(eq(invoices.id, postedInvoice.id));
                }
            }
