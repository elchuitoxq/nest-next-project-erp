...
import { db, payrollRuns, payrollItems, employees, currencies, payrollIncidents, payrollConceptTypes } from '@repo/db';
...
export class PayrollService {
  ...
  async generate(data: {
    ...
    if (!vesCurrency) throw new BadRequestException('Moneda VES no configurada');
    ...
    if (activeEmployees.length === 0) {
        throw new BadRequestException('No hay empleados activos para esta frecuencia de pago');
    }
    ...
    const incidentsMap = new Map<string, { income: Decimal, deduction: Decimal }>();
    
    for (const incident of incidents) {
      if (!incidentsMap.has(incident.employeeId)) {
        incidentsMap.set(incident.employeeId, { income: new Decimal(0), deduction: new Decimal(0) });
      }
    ...
    for (const emp of activeEmployees) {
        ...
        // Add Incidents
        const incidentData = incidentsMap.get(emp.id) || { income: new Decimal(0), deduction: new Decimal(0) };
        ...
        // Total for Employee
        const netTotal = finalBaseAmountVes.plus(totalBonuses).minus(totalDeductions);
    ...
    return await db.transaction(async (tx) => {
        ...
        if (itemsToInsert.length > 0) {
            await tx.insert(payrollItems).values(
                itemsToInsert.map(item => ({ ...item, runId: run.id }))
            );
        }
        ...
        if (includedIncidentIds.length > 0) {
            await tx.update(payrollIncidents)
