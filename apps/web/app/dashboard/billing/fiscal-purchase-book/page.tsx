...
export default function FiscalPurchaseBookPage() {
  const [month, setMonth] = useState<string>(
    (new Date().getMonth() + 1).toString(),
  );
  const [year, setYear] = useState<string>(new Date().getFullYear().toString());

  const { data, isLoading } = useFiscalBook(
    Number(month),
    Number(year),
    "PURCHASE",
  );
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Libro de Compras</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
        <div className="space-y-6">
          <DashboardHeader
            ...
            description="Reporte fiscal mensual de compras y créditos fiscales (Formato SENIAT)."
          >
