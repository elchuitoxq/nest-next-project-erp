...
Skills are `.md` files located in `.agent/skills/`. To use a skill:

1. **Identify** which skill might apply to your current task.
2. **Read** the `SKILL.md` file using your file reading capabilities.
...
**Read and follow relevant skills BEFORE any response or action.** Even a 1% chance a skill might apply means you should read the skill to check. If a read skill turns out to be wrong for the situation, you don't need to use it.

```dot
digraph skill_flow {
    "User message received" [shape=doublecircle];
    "Might any skill apply?" [shape=diamond];
    "Read SKILL.md file" [shape=box];
    ...
    "Might any skill apply?" -> "Read SKILL.md file" [label="yes, even 1%"];
    "Might any skill apply?" -> "Respond (including clarifications)" [label="definitely not"];
    "Read SKILL.md file" -> "Announce: 'Using [skill] to [purpose]'";
